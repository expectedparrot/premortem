...
def ingest_personas_cmd(
    ...
    if json_flag:
        finish(command, {"created": len(created), "personas": [p.model_dump(mode="json") for p in created]}, True, quiet)
        return
    if not quiet:
...
def ingest_reasons_cmd(
    ...
    if json_flag:
        finish(
            command,
            {"created": len(created), "reasons": [r.model_dump(mode="json") for r in created]},
            True,
            quiet,
            warnings=warnings,
        )
...
def ingest_scores_cmd(
    ...
    try:
        ...
        fail(command, err, json_flag)
    if json_flag:
        finish(command, {"created": len(created), "scores": [s.model_dump(mode="json") for s in created]}, True, quiet)
        return
    if not quiet:
        from ..renderer import console
...
def ingest_mitigations_cmd(
    ...
    try:
        ...
        fail(command, err, json_flag)
    if json_flag:
        finish(command, {"created": len(created), "mitigations": [m.model_dump(mode="json") for m in created]}, True, quiet)
        return
    if not quiet:
        from ..renderer import console
